...
![Bistro Exterior を ReSTIR GI + DI で表示し、MCP Server panel を開いた D3D12LookDevPT](images/screenshot005-mcp-server-runtime.png)

![Bistro Interior を ReSTIR GI + DI で表示し、ReSTIR panel を開いた D3D12LookDevPT](images/screenshot006-bistro-interior-restir.png)

上の screenshot は local test asset として配置した Bistro Exterior / Bistro Interior scene を使っています。大きな scene asset は repository に含めない方針です。配置方法は [アセットの配置](docs/assets.ja.md) を参照してください。
...
- optional NVIDIA Streamline / DLSS Ray Reconstruction backend。未対応 GPU や runtime DLL 不足時は internal denoiser に fallback。
...
DLSS を厳密に確認する場合は `.\Scripts\CheckSetup.ps1 -CheckDLSS` を使います。DLSS を完全に外して build する場合は MSBuild に `/p:EnableDLSS=false` を指定してください。
...
- `ThirdParty/Streamline`: NVIDIA Streamline SDK。DLSS を使わない build では `/p:EnableDLSS=false` で不要化できます。
- `ThirdParty/DLSS`: NVIDIA DLSS SDK。DLSS runtime の扱いは [Optional DLSS Ray Reconstruction](docs/dlss.ja.md) を参照してください。
...
MCP では project save/load、camera fitting、display resolution 変更、reset tools、debug-view capture pack、material texture / variant / color management 操作、capture resources、prompts も使えます。VS Code 設定、JSON-RPC 例、tools/resources/prompts、MCP 経由の camera / denoise 操作 screenshot、troubleshooting は [MCP サーバー詳細](docs/mcp.ja.md) を参照してください。
...
- `lookdevpt.list_debug_views`
- `lookdevpt.list_render_modes`
- `lookdevpt.get_diagnostics`
- `lookdevpt.capture_viewport`
- `lookdevpt.capture_debug_pack`
- `lookdevpt.validate_action`
- `lookdevpt.run_actions`
